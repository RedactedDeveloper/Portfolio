###Creator of Worlds V.0.1.4###
#29/11/21#

##The dreaded U.I overhaul##

#Saving should warn the user about overwriting -Function is already made, add it to the saving. Done
...
#Block cannot be placed, user is warned, but only once and the message never disapears. Done (Needs U.I redoing, looks bad)
...
class LevelCreatorView(arcade.View):
    ...
    def on_update(self, dt):
        ...
        try:
            ...
            if (self.manager[1]) == self.warn_StatementOverhead:

                self.count_For_Warn_Statement_Overhead += 1

                if self.count_For_Warn_Statement_Overhead == 50:

                    self.manager.remove(self.warn_StatementOverhead)

            elif (self.manager[1]) == self.warn_Statement:

                self.count_For_Warn_Statement += 1

                if self.count_For_Warn_Statement == 50:

                    self.manager.remove(self.warn_Statement)
        except:
            pass

        ##Try indexing into te self.manager to find all the instances it happens.
        ##This solution doesnt account for multiple instances of the text.
        ##Either continue with this, or
        ##Have a pop up box the user has to press a button on using the arcade.gui.UIMessageBox. Less ideal but easier.
...
##            self.level_Button = arcade.gui.UIFlatButton(x, y, 300, 75, level)
##            self.level_Button.on_click = self.level_Play(event="on_click", level=level)
##            self.manager.add(self.level_Button)
##            x += 30
##            y += 30
        #This clicks when it first calls. It calls the function with a click command. So it thinks it has been.
        try:

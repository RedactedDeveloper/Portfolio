###Creator of Worlds V.0.1.5###
#14/12/21#

##Camera Fuctionality in the platformer##

#Make sure camera doesnt go off the right side
#Make the GUI camera follow the normal camera
#Camera functionality fixed in level creator, tiles stopped working briefly.
...
SCREEN_WIDTH = 1280
SCREEN_HEIGHT = 720
SCREEN_TITLE = "Creator of Worlds: Camera"
...
##Either we pass in the level width and height here.
#Or pass in a value that determines the size as the user will only have 3 options to choose from.
#Then have a small either list or dictionary. - List because the height stays the same.

class LevelCreatorView(arcade.View):

    def __init__(self, levelloadquery, level_Size=1):

        super().__init__()
        self.level_Size = level_Size
        self.level_Size_Data =  [1280, 2560, 3840]
    ...
    def setup(self):
        ...
        self.camera_Sprite = arcade.Sprite(r"C:\Users\AToll\OneDrive\Desktop\LevelCreator\Assets\Textures\BlankGrid.PNG",
                                           center_x=630, center_y=350)

        self.player_Camera = arcade.Camera(1280, 720)

        self.gui_camera = arcade.Camera(SCREEN_WIDTH, SCREEN_HEIGHT)

        empty = arcade.SpriteList()

        self.physics_engine = arcade.PhysicsEngineSimple(self.camera_Sprite,
                                                             empty)
        ...
        if self.levelloadquery == 1:
            ...
            totalneeded = ((self.level_Size_Data[self.level_Size] * self.level_Height) / (GRID_SQUARE_SIZE * GRID_SQUARE_SIZE))
            #print(totalneeded)
            ...
            for gridspot in range(0, totalneeded):
                ...
                if START_X >= self.level_Size_Data[self.level_Size]:
                    START_X = 10
                    START_Y += 20
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == (arcade.key.A):
            if self.camera_Sprite.center_x > 630:
                self.camera_Sprite.change_x = -self.camera_Speed

        if key == (arcade.key.D):
            if self.camera_Sprite.center_x < (self.level_Size_Data[self.level_Size] - 660):
                self.camera_Sprite.change_x = self.camera_Speed
...
##        if self.camera_Sprite.center_x > 630:
##            print(self.camera_Sprite.center_x)
##            if len(gridspot) > 0:
##                old_Pos = gridspot[0].position
##                new_Pos = list(old_Pos)
##                new_Pos[0] += (self.camera_Sprite.center_x - 630)
##                new_Pos = tuple(new_Pos)
##                print(new_Pos)
##                gridspot[0].position = (new_Pos)
        ...
        if button == arcade.MOUSE_BUTTON_LEFT:
            ...
            if self.camera_Sprite.center_x > 630:
                if len(gridspot) > 0:
                    ...
                    new_GridNumber = calc_Grid_Spot(new_Pos[0], new_Pos[1])

                    for i in range(0, len(self.gridamount)):
                        if self.gridamount[i].gridnumber == (new_GridNumber):
                            gridspot = []
                            gridspot.append(self.gridamount[i])
                            break
    ...
    def camera_movement(self):
        screen_center_x = self.camera_Sprite.center_x - (self.player_Camera.viewport_width / 2)
        
        screen_center_y = self.camera_Sprite.center_y - (self.player_Camera.viewport_height / 2)
        ...
        self.player_Camera.move_to(camera_centered)
        #print(self.player_Camera.position)
...
class PlatformerView(arcade.View):
    ...
    def setup(self):
        ...
        self.camera = arcade.Camera(1280, 720)
        self.gui_Camera = arcade.Camera(SCREEN_WIDTH, SCREEN_HEIGHT)
        self.score = 0
    ...
    def move_camera_to_player(self):
        ...
        ###Add the functionality of it finds the level edge and won't move the camera past it###
        ##Either I save the level size and use that, but have to change the level loading and saving,
        #Or work it out using the last line
